...
class ImageNarrator(object):
    ...
    def get_result(self):
        ...
        for caption in caption_obj_list:
            pprint(len(caption_obj_list))
            self.charnn([caption])
            # self.process_to_html()
            return self.expansion_obj_list
    ...
    def process_to_html(self):
        img_fps = map(
            lambda x: os.path.join('/~anqi', 'imgs', 'img%s.jpg'%x['id']),
        ...
        html_fp = '/home/anqi/public_html/%s%s.html' % (re.sub(r'\W+', '_', self.output_title), self.expansion_obj_list[0]['id'])
